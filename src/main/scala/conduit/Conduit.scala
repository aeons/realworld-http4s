...
import conduit.algebra.UserAlgebra
import conduit.model.{DbUser, LoginRequest, RegistrationRequest, User}
import conduit.types.{Email, Username}
import fs2._
import org.http4s._
...
object Conduit {
  ...
  def registerUser[F[_]](users: UserAlgebra[F], email: Email, username: Username, password: String)(
      implicit dsl: Http4sDsl[F]): F[Response[F]] = {
    import dsl._
    // users.register(email, username, password).map(_.fold(BadRequest))
    ???
  }
    ...
    HttpService[F] {
      ...
      case req @ POST -> Root / "users" / "register" =>
        for {
          ...
          else registerUser(reg.email, reg.username, reg.password)
        } yield resp
    }
  }
